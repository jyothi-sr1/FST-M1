...
public class Activity2 {
	...
	public void addNewUserFromFile() throws IOException {
		...
		response.then().body("message", equalTo("99110"));
	}
	...
	public void getUserInfo() {
		...
		Response response = given()
			.baseUri("https://petstore.swagger.io/v2/user") // Set base URI
			.header("Content-Type", "application/json") // Set headers
			.pathParam("username", "justing") // Pass request body from file
		...
		// Assertion
		response.then().body("id", equalTo(99110));
		response.then().body("username", equalTo("justing"));
	...
	public void deleteUser() throws IOException {
		Response response = given()
			.baseUri("https://petstore.swagger.io/v2/user") // Set base URI
			.header("Content-Type", "application/json") // Set headers
			.pathParam("username", "justing") // Add path parameter
			.when().delete("/{username}"); // Send POST request

		// Assertion
		response.then().body("code", equalTo(200));
		response.then().body("message", equalTo("justing"));
	}
}
